...
void findInverse(int m[3][3], int n, int m_inverse[3][3])
{
	...
	cout << "Determinant Inverse: " << detInverse << "\n";
	...
	//Tried for Key with dimension 3*3...
	// else if (n == 3)
	// {
	// 	int temp[5][5] = {0};
	// 	// fill the 5x5 matrix
...
string decrypt(string ct, int n)
{
	...
	int key_inverse[3][3] = {0};
	/* findInverse(matrix , order_of_matrix , result_matrix) */
	findInverse(key, n, key_inverse);
	cout << "\nKey Inverse: \n";
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
	...
	cout << '\n';
	/* multiplyMatrices(mat_a , row_a , col_a , mat_b, row_b, col_b , mat_result) */
	multiplyMatrices(C, row, n, key_inverse, n, n, P);
